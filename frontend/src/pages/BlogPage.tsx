...
import { BlogPost } from '../types'; // We'll create this type definition
...
const BlogPage = () => {
    ...
    if (loading) {
        return <div className="min-h-screen bg-quantum-black flex items-center justify-center text-soft-white">Loading...</div>;
    }
                            ...
                            <Link to={`/blog/${post.id}`} key={post.id}>
                                <Card className="overflow-hidden h-full flex flex-col group transform hover:-translate-y-2 transition-transform duration-300">
                                    ...
                                    <div className="p-6 flex-grow flex flex-col">
                                        <h2 className="text-2xl font-bold text-soft-white mb-2 group-hover:text-ion-blue transition-colors">{post.title}</h2>
                                        <p className="text-gray-400 line-clamp-3 flex-grow">{post.content}</p>
